...
async function run() {
    ...
    // 1. Move CH202512252011 back to pending and remove driver
    const { error: e1 } = await supabase.from('orders')
        .update({ status: 'pending', driver_id: null })
        .eq('note', '測試訂單 [ID: CH202512252011]');

    // 2. Fix CH9999 status to 'confirmed'
    const { error: e2 } = await supabase.from('orders')
        .update({ status: 'confirmed', driver_id: '05a8b68d-7159-43a1-8cb2-2afb1bc51700' }) // Assigned to Sun Xiao-mei
        .eq('note', '[ID: CH9999]');

    if (e1 || e2) console.error('Error resetting:', e1, e2);
    else console.log('Successfully reset orders. CH202512252011 is now "Pending" (Unconfirmed).');
}
run();
